...
export default function DerivativesPage() {
  ...
  const columns: Column<Pair>[] = [
    ...
    { key: 'volume24h', header: 'Volume', width: 100, align: 'right', render: r => <span className="text-text-secondary">${((r.volume24h ?? 0) / 1e6).toFixed(1)}M</span> },
    ...
    { key: 'fundingRate', header: 'Funding', width: 70, align: 'right', render: r => <span className={`font-mono ${r.fundingRate > 0 ? 'text-data-bull' : r.fundingRate < 0 ? 'text-data-bear' : 'text-text-muted'}`}>{r.fundingRate !== 0 ? `${((r.fundingRate ?? 0) * 100).toFixed(4)}%` : '—'}</span> },
